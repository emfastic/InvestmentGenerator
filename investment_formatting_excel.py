...
def sheet_selection(workbook):
    """ Asks the user for what sheet to edit if not creating a new one, returns the sheet and sheet_name """
    while True:
        new_sheet = input('Would you like to create a new sheet (y/n): ')
        sheet_name = None
        sheet = None
        if new_sheet.strip().lower() == 'y':
            print('What would you like the name of the sheet to be?')
            sheet_name = input('> ')
            workbook.create_sheet(f'{sheet_name}')
            sheet = workbook[f'{sheet_name}']
            return sheet, sheet_name
        if new_sheet.strip().lower() == 'n':
            print('What sheet would you like to edit?')
            edit_existing = input('> ')
            sheet_name = edit_existing.strip().title()
            sheet = workbook[f'{sheet_name}']
            return sheet, sheet_name
...
def create_stock_obj_list(stock_list):
...
def edit_spreadsheet(workbook_name):
    """ Creates investment.xlsx spreadsheet and formats in the desired data """
    workbook = load_workbook(f'{workbook_name}')

    sheet, sheet_name = sheet_selection(workbook)
    stock_amt = stock_number(sheet_name)
    stock_list = create_stock_list(stock_amt)
    years_of_data = years_back()
    stock_obj_list = create_stock_obj_list(stock_list)
    row_num = current_row(sheet) + 1 # to start on the first blank row
    header_list = create_header_list(stock_obj_list, years_of_data)

    if row_num == 2:
        sheet.insert_rows(idx=1, amount=len(stock_obj_list) + 1)
        create_headers(sheet, header_list)
        create_stock_rows(sheet, stock_obj_list, row_num, years_of_data)
    else:
